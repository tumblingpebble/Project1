...
data = pd.read_csv("../data/processed/ad_click_dataset_imputed.csv")
...
import joblib
joblib.dump(rf_model, '../models/rf_model_groupmate.pkl')

# Save predictions and probabilities for later use in visualizations
pd.DataFrame(y_test_pred, columns=['Predictions']).to_csv('../predictions/random_forest_test_predictions_groupmate.csv', index=False)
...
plt.grid()
plt.legend(loc='best')
plt.savefig('../visualizations/learning_curve_rf_groupmate.png')
plt.close()
...
plt.ylabel("Actual")
plt.savefig('../visualizations/confusion_matrix_rf_groupmate.png')
...
plt.grid()
plt.savefig('../visualizations/roc_curve_rf_groupmate.png')
...
plt.grid()
plt.legend()
plt.savefig('../visualizations/precision_recall_rf_groupmate.png')
